...
import { Firestore, collection, collectionData, doc, addDoc, deleteDoc, query, orderBy, limit, limitToLast } from '@angular/fire/firestore';
...
export class DashboardComponent {
  ...
  constructor(private firestore: Firestore, public http: HttpClient, public snackBar: MatSnackBar, public sanitizer: DomSanitizer) {
    ...
    console.log('dashboard contrsuctor')

    const entriesQuery = query(collection(this.firestore, 'entries'), orderBy('meta.createdate'), limit(2));
    collectionData(entriesQuery, { idField: 'id' }).subscribe(x => {
      this.entries = x;
    });

    const logsQuery = query(collection(this.firestore, 'logs'), orderBy('datetime'), limitToLast(10));
